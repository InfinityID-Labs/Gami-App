...
persistent actor UserProfiles {
  ...
  // State
  private var profilesEntries : [(UserId, UserProfile)] = [];
  private transient var profiles = HashMap.HashMap<UserId, UserProfile>(50, Principal.equal, Principal.hash);
  // Restaura os dados persistidos
  for ((k, v) in profilesEntries.vals()) {
    profiles.put(k, v);
  };

  private var achievementsEntries : [(Text, Achievement)] = [];
  private transient var achievements = HashMap.HashMap<Text, Achievement>(20, Text.equal, Text.hash);
  for ((k, v) in achievementsEntries.vals()) {
    achievements.put(k, v);
  };

  // System functions
  system func preupgrade() {
    var tempProfiles : [(UserId, UserProfile)] = [];
    for (entry in profiles.entries()) {
      tempProfiles := Array.append(tempProfiles, [entry]);
    };
    profilesEntries := tempProfiles;
    var tempAchievements : [(Text, Achievement)] = [];
    for (entry in achievements.entries()) {
      tempAchievements := Array.append(tempAchievements, [entry]);
    };
    achievementsEntries := tempAchievements;
  };
  ...
  public query func getUserAchievements(userId : UserId) : async [Achievement] {
    switch (profiles.get(userId)) {
      ...
      case (?profile) {
        ...
        |> Array.mapFilter(
          _,
          func(achievementId : Text) : ?Achievement {
  ...
  public query func getGlobalStats() : async {
    ...
    for (p in profiles.vals()) {
      allProfiles := Array.append(allProfiles, [p]);
    };
